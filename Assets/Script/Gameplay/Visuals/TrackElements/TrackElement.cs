...
namespace YARG.Gameplay.Visuals
{
        ...
        where TPlayer : TrackPlayer
    {
        ...
        protected float GetZPositionAtTime(double time)
        {
            ...
            // critical areas such as the game manager and players. EffectiveVisualTime is used
            // instead of GameManager.VisualTime so remote-peer highways are shifted back by the
            // prediction layer's visual delay — without that, notes would cross the strikeline at
            // local song time but the engine wouldn't commit them until commit-window later.
        ...
        {
            // Calibration is not taken into consideration here, as that is instead handled in more
            // critical areas such as the game manager and players. EffectiveVisualTime — see
            // GetZPositionAtTime above.
            float z =
